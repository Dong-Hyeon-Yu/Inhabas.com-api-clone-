...
import com.inhabas.api.members.domain.entity.questionaire.Questionnaire;
import com.inhabas.api.members.domain.entity.questionaire.QuestionnaireRepository;
...
import testConfig.CustomSpringBootTest;
...
public class SignUpIntegrationTest {
    ...
    public void OAuth2_인증_후_비회원_신규_학생_회원가입() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/signUp/student").with(accessToken(token)))
                        .andExpect(MockMvcResultMatchers.status().isOk())
                        .andExpect(MockMvcResultMatchers.content().string("{\"email\":\"my@gmail.com\"}"));

        /* 개인정보 입력을 위해, 전공 정보들이 로딩된다. */
        mockMvc.perform(MockMvcRequestBuilders.get("/signUp/majorInfo").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isOk());

        /* 프로필 입력 중에 학번이 중복되는 지 검사한다.
        중복되는 학번이 없다고 응답한다. */
        mockMvc.perform(MockMvcRequestBuilders.get("/signUp/isDuplicated")
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/signUp/student").with(accessToken(token))
        ...
        String questionList = mockMvc.perform(MockMvcRequestBuilders.get("/signUp/questionnaire").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andReturn().getResponse().getContentAsString(StandardCharsets.UTF_8);
        Assertions.assertThat(questionList).isEqualTo("[" +
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/signUp/answer").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().string("[]"));

        /* 면접 질문에 대답하고 임시저장 버튼을 클릭한다. */
        mockMvc.perform(MockMvcRequestBuilders.post("/signUp/answer").with(accessToken(token))
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        /* 회원가입 신청을 완료한다. */
        mockMvc.perform(MockMvcRequestBuilders.put("/signUp/finish").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isNoContent());
        ...
        Assertions.assertThat(유동현_소셜_계정.getProfileId()).isEqualTo(12171652);
        Assertions.assertThat(유동현_소셜_계정.hasJoined()).isEqualTo(true);
    }
    ...
    public void OAuth2_인증_후_비회원_신규_교수_회원가입() throws Exception {
        ...
        /* 개인정보 입력을 위해, 전공 정보들이 로딩된다. */
        String majorList = mockMvc.perform(MockMvcRequestBuilders.get("/signUp/majorInfo").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andReturn().getResponse().getContentAsString(StandardCharsets.UTF_8);
        Assertions.assertThat(majorList).isEqualTo("[" +
        ...
        /* 프로필 입력을 완료하여 다음 버튼을 누르면, 개인정보가 임시저장된다. */
        mockMvc.perform(MockMvcRequestBuilders.post("/signUp/professor").with(accessToken(token))
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        AuthUser 유동현_소셜_계정 = authUserRepository.findById(authUserId).orElseThrow(AuthUserNotFoundException::new);
        Assertions.assertThat(유동현_소셜_계정.getProfileId()).isEqualTo(228761);
        Assertions.assertThat(유동현_소셜_계정.hasJoined()).isEqualTo(true);
    }
    ...
    private void forbiddenWhenAccessEverySignUpApi(Role role) throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/signUp/answer").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isForbidden());
        mockMvc.perform(MockMvcRequestBuilders.get("/signUp/questionnaire").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isForbidden());
        mockMvc.perform(MockMvcRequestBuilders.get("/signUp/majorInfo").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isForbidden());
        mockMvc.perform(MockMvcRequestBuilders.get("/signUp/isDuplicated").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isForbidden());
        mockMvc.perform(MockMvcRequestBuilders.put("/signUp/finish").with(accessToken(token)))
                .andExpect(MockMvcResultMatchers.status().isForbidden());
    }
